...
public class WechatController {
    ...
    public String userInfo(@RequestParam("code") String code,
        ...
        log.info("openid={},nickname={},headImgUrl={}",
    ...
    @GetMapping("/authorize2token")
    public String Authorize2token(@RequestParam("returnUrl") String returnUrl) {
        //1. 配置
        //2. 调用方法
        String url = projectUrlConfig.getWechatMpAuthorize() + "/sell/wechat/userInfo2token";
        String redirectUrl = wxMpService.oauth2buildAuthorizationUrl(url, WxConsts.OAUTH2_SCOPE_BASE, URLEncoder.encode(returnUrl));
        return "redirect:" + redirectUrl;
    }

    @GetMapping("/userInfo2token")
    public String userInfo2token(@RequestParam("code") String code,
                           @RequestParam("state") String returnUrl) {
        WxMpOAuth2AccessToken wxMpOAuth2AccessToken = new WxMpOAuth2AccessToken();
        try {
            wxMpOAuth2AccessToken = wxMpService.oauth2getAccessToken(code);
        ...
        //设置token至redis
        String token = userTokenService.setToken(openId);
